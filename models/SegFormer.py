...
Adapted for parameter-shared multi-modal training (SAR/RGB) with 4x downsampling stem

Key Changes:
- UnifiedMultiModalStem now performs 4x downsampling (stride=4)
- First patch_embed adjusted to account for already downsampled input
...
class MixTransformer(nn.Module):
    ...
    def __init__(self, img_size=512, in_chans=64, num_classes=1, 
        ...
        # ✅ 修改: patch_embed1 的 stride=1，因为输入已经被 stem 下采样 4x
        # 输入尺寸已经是 img_size//4，所以不需要再次下采样
        self.patch_embed1 = OverlapPatchEmbed(img_size=img_size // 4, patch_size=7, stride=1, 
...
class SegFormer(nn.Module):
    ...
    3. Stem 执行 4x 下采样，减少显存占用
    4. 支持交替批次训练（每个batch单一模态）
    """
    def __init__(self, 
                 img_size=512,
                 num_classes=1,
        ...
        # ✅ 输入适配层：将不同模态统一到相同通道数，并执行 4x 下采样
    ...
    def forward(self, x, datasets=None):
        ...
        # ✅ Step 1: 通过输入适配层统一通道数并下采样 4x
        #    自动根据输入通道数选择正确的对齐分支
        #    输出: [B, stem_channels, H//4, W//4]
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("SegFormer with 4x Downsampling Stem")
    print("="*60)
