...
class UNetBlock(nn.Module):
    ...
    def __init__(self,
        ...
        # UNet 标准双卷积结构
        self.conv1 = nn.Conv2d(in_channels, in_channels, kernel_size=3, padding=1, groups=1)
        self.norm1 = self._build_norm(norm_cfg, in_channels)
        self.act = build_activation_layer(act_cfg)

        self.conv2 = nn.Conv2d(in_channels, in_channels, kernel_size=3, padding=1, groups=1)
...
class BasicUNet(BaseModule):
    ...
    def __init__(self,
                 ...
                 in_channels=3,
                 stem_patch_size=4,
                 norm_cfg=dict(type='BN'),  # UNet通常使用BN
                 act_cfg=dict(type='ReLU'),  # UNet通常使用ReLU
                 linear_pw_conv=True,
                 use_grn=False,
                 drop_path_rate=0.,
                 layer_scale_init_value=1e-6,
    ...
    def train(self, mode=True):
        super(BasicUNet, self).train(mode)
        self._freeze_stages()
